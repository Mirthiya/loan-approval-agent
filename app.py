...
def decide(app, model, FN, FM, repo_rate=6.50):
    ...
    # Decision — 3 tiers:
    # REJECTED:      very high ML risk OR 2+ rule failures OR critical failure + high risk
    # MANUAL REVIEW: borderline ML risk OR any 1 rule failure OR critical failure + low risk
    # APPROVED:      passes all rules cleanly with low ML risk
    if prob > 0.55 or len(failed) >= 2 or (cf and prob > 0.35):
        dec = "REJECTED";      aa = 0;       ir = None
    elif prob > 0.20 or len(failed) >= 1 or cf:

...
def parse_row(row):
    ...
    # Build a stable unique ID from the row's own data (name + email + loan amount).
    # Do NOT use datetime.now() as fallback — all 14 rows processed in the same
    # second would get identical IDs and be skipped as "already processed".
    _name  = str(row.get("Full Name", row.get("Name", row.get("name", "")))).strip()
    _email = str(row.get("Email Address", row.get("Email", row.get("email", "")))).strip()
    _loan  = str(row.get("Loan Amount (Rs)", row.get("Loan Amount",
                 row.get("Loan amount", row.get("loan amount", ""))))).strip()
    _ts    = str(row.get("Timestamp", ""))  # use sheet timestamp if available

    if _ts:
        # Sheet has a Timestamp column — use it combined with name for uniqueness
        raw = (_ts + "_" + _name).replace(" ", "_").replace(":", "").replace("/", "")
    elif _name and _email:
        # No timestamp — build from name + email + loan (stable across re-reads)
        import hashlib
        raw = hashlib.md5((_name + _email + _loan).encode()).hexdigest()[:16]
    else:
        # Last resort: row index + current time (guarantees uniqueness this cycle)
    ...
    # Check whether 3 *separate* bureau columns actually exist in this row
    _has_cs1 = any(k in row for k in ("Credit Score 1", "Credit score 1"))
    _has_cs2 = any(k in row for k in ("Credit Score 2", "Credit score 2"))
    _has_cs3 = any(k in row for k in ("Credit Score 3", "Credit score 3"))
    has_3_credit_cols = _has_cs1 and _has_cs2 and _has_cs3
    ...
    return {
        ...
        "has_3_credit_cols" : has_3_credit_cols,   # True only if sheet had 3 bureau columns
